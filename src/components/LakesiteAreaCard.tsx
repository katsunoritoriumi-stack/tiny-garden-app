...
import type { Area, CheckInInfo, CleanStatus, KeyStatus } from '../types'
...
export default function LakesiteAreaCard({
  area, onToggleTask, onSetCheckInInfo, onSetKeyStatus, onSetCleanStatus, onSetNote,
}: Props) {
        ...
        return (
          <div key={room.id}>
            ...
            <div style={{ opacity: done ? 0.5 : 1, transition: 'opacity 0.3s' }}>
              ...
              <div style={{ marginBottom: '10px' }}>
                <button
                  onClick={() => onSetCleanStatus(room.id, cleanStatus === 'needed' ? 'unset' : 'needed')}
                  style={{
                    width: '100%',
                    padding: '8px 14px',
                    minHeight: '44px',
                    border: `2px solid ${cleanStatus === 'needed' ? '#e67e22' : 'var(--border)'}`,
                    borderRadius: '8px',
                    background: cleanStatus === 'needed' ? 'rgba(230,126,34,0.2)' : 'transparent',
                    color: cleanStatus === 'needed' ? '#e67e22' : 'var(--text-secondary)',
                    fontSize: '14px',
                    fontWeight: 700,
                    cursor: 'pointer',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                ...
                <button
                  ...
                  style={{
                    flex: 1, padding: '8px', minHeight: '40px',
